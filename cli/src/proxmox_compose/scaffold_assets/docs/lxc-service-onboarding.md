...
5. Optional (Python apps): define `lxc_python_apps` entries to create a venv, install editable dependencies, and run migrations.
...
6. Optional: define `lxc_runtime_dirs` entries (`path`, `owner`, `group`, `mode`) for state / cache / output directories the service user must own (created before the unit starts; use for paths referenced in `EnvironmentFile=`).
7. Define `lxc_systemd_services` entries with `exec_start`; optional `environment_file` / `environment_files` for systemd `EnvironmentFile=`.
8. Re-run `proxmox-compose apply`.
